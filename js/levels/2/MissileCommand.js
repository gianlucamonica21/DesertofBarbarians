...
var createEmemyMissiles = function() {
  var targets = viableTargets(),
    numMissiles = /*Increased since last level*/ 5;
...
function userSolutionChecker() {
  ...
  playerShoot(70, 350);
  try {
    var x = playerMissiles[playerMissiles.length - 1].endX;
    var y = playerMissiles[playerMissiles.length - 1].endY;
    if (x == 70 && y == 350) {
      return {
        passed: true,
        msg: "Great! Everything works again! Not bad for your first mission, newbie. Thanks to you, we are now able to counter-attack!"
      }
    } else {
      return {
        passed: false,
        msg: "Does it seem like the problem is fixed? <b>TRY HARDER</b> !"
      }
    }
  } catch (err) {
    if (err.name === "TypeError") {
      writeChatMessage("Please try out the your solution before validating!", "soldierMsg");
...
=======
  ...
  if(err.name === "TypeError"){
    ...
    return {
      passed: false,
      msg: "Please try out the your solution before validating!"
    }
  }
}
>>>>>>> e52b7b1b1936334d96beb44582153aa2a9c41079
